...
## Future directions for the interface

The uppaal command line interface is not finished yet, several interesting directions could be further explored to make it more interesting. In particular the following aspects should be of direct interest:

- saving / loading a strategy, the command is already implemented but for some reason it does not seam to work with the uppaal server;

- if and for statements: as you will see in the file train-gate.upl the verification is started at the end of the main script but for the sake of modularity it could be interesting to introduce if statements to avoid this verification when the script is not the main one;

- further improvements suggested by users …

## Architecture

The uppaal command line interface is organized according to a MVC architecture adapted to the command line:
